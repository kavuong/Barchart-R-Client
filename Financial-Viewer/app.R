...
server <- function(input, output) {
  ...
  chosenForex = reactive({
    c(ForexChosen = paste("^",input$currencyFrom, input$currencyTo, sep=""), SelectTimeForex = input$selectTimeForex,
  ...
  observeEvent(input$submitEquity, {
    ...
    output$stockPlot <- renderPlot({
        ggplot(data.to.display, aes(x=as.character(timestamp), y=open, group = 1)) + geom_line() + xlab("Time") + ylab("Price ($)") + ggtitle(paste(data$results$symbol[1], "in the past", time.text, sep = " ")) + scale_x_discrete(breaks = my_breaks, labels = my_labels)
      })
    }
    else {
      output$stockPlot <- renderPlot({
        ggplot(data.to.display) + geom_col(aes(x=as.character(timestamp), y=volume)) + xlab("Time") + ylab("Volume") + ggtitle(paste(data$results$symbol[1], "Volume in the past", time.text, sep = " ")) + scale_x_discrete(breaks = my_breaks, labels = my_labels)
      })
    }
  }
  )
  ...
  observeEvent(input$submitFuture, {
    ...
    output$futureLabel <- renderText(paste("Stock Symbol: ", {data$results$symbol[1]}))
    output$futureCurrentPrice <- renderText({paste("Current Price: ", data$results$lastPrice[1])})
    output$futureOpenPrice <- renderText({paste("Open: ", data$results$open[1])})
    output$futureHighPrice <- renderText({paste("Today's High: ", data$results$high[1])})
    output$futureLowPrice <- renderText({paste("Today's Low: ", data$results$low[1])})
    output$future52WeekLow <- renderText({paste("52-wk Low:", data$results$fiftyTwoWkLow[1])})
    output$future52WeekLowDate <- renderText({paste("52-wk Low on: ", data$results$fiftyTwoWkLowDate[1])})
    output$future52WeekHigh <- renderText({paste("52-wk High: ", data$results$fiftyTwoWkHigh[1])})
    output$future52WeekHighDate <- renderText({paste("52-wk High on: ", data$results$fiftyTwoWkHighDate[1])})
  ...
  observeEvent(input$submitForex, {
    ...
    test.data <- GET(
      ...
      query = list(
        apikey = Sys.getenv("BARCHART_TOKEN"),
        symbols = {chosenForex()["ForexChosen"]},
        fields = c("fiftyTwoWkHigh,fiftyTwoWkHighDate,fiftyTwoWkLow,fiftyTwoWkLowDate")
      )
    )
    ...
    output$forexLabel <- renderText(paste("Stock Symbol: ", {data$results$symbol[1]}))
    output$forexCurrentPrice <- renderText({paste("Current Price: ", data$results$lastPrice[1])})
    output$forexOpenPrice <- renderText({paste("Open: ", data$results$open[1])})
    output$forexHighPrice <- renderText({paste("Today's High: ", data$results$high[1])})
    output$forexLowPrice <- renderText({paste("Today's Low: ", data$results$low[1])})
    output$forex52WeekLow <- renderText({paste("52-wk Low:", data$results$fiftyTwoWkLow[1])})
    output$forex52WeekLowDate <- renderText({paste("52-wk Low on: ", data$results$fiftyTwoWkLowDate[1])})
    output$forex52WeekHigh <- renderText({paste("52-wk High: ", data$results$fiftyTwoWkHigh[1])})
    output$forex52WeekHighDate <- renderText({paste("52-wk High on: ", data$results$fiftyTwoWkHighDate[1])})
    ...
    test.data.history <- GET(
      ...
      query = list(
        apikey = Sys.getenv("BARCHART_TOKEN"),
        symbol = {chosenForex()["ForexChosen"]},
        type = inType,
        startDate = "20200101",
        interval = interv,
    ...
    if (chosenForex()["SelectFormatForex"] == "Price($)"){
      output$forexPlot <- renderPlot({
        ggplot(data.to.display, aes(x=as.character(timestamp), y=open, group = 1)) + geom_line() + xlab("Time") + ylab(paste("Price of", ex.display, sep=" ")) + ggtitle(paste(ex.display, "in the past", time.text, sep = " ")) + scale_x_discrete(breaks = my_breaks, labels = my_labels)
      })
    }
    else {
      output$forexPlot <- renderPlot({
        ggplot(data.to.display, aes(x=as.character(timestamp), y=volume, group = 1)) + geom_col() + xlab("Time") + ylab(paste("Volume of", ex.display, sep=" ")) + ggtitle(paste(ex.display, "Volume in the past", time.text, sep = " ")) + scale_x_discrete(breaks = my_breaks, labels = my_labels)
        })
    }
    output$forexResult <- renderText(paste(input$amountCurrencyFrom, input$currencyFrom, "is equal to", {input$amountCurrencyFrom * data$results$lastPrice[1]}, input$currencyTo, sep = " "))
  }
  )
}
